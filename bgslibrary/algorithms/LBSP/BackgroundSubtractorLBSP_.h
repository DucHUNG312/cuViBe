...
namespace lbsp {
/*!
    Local Binary Similarity Pattern (LBSP)-based change detection algorithm
   (abstract version/base class).

    For more details on the different parameters, see P.-L. St-Charles and G.-A.
   Bilodeau, "Improving Background Subtraction using Local Binary Similarity
   Patterns", in WACV 2014, or G.-A. Bilodeau et al, "Change Detection in
   Feature Space Using Local Binary Similarity Patterns", in CRV 2013.
...
public:
  //! full constructor
  BackgroundSubtractorLBSP_(float fRelLBSPThreshold,
                            size_t nLBSPThresholdOffset = 0);
  //! default destructor
  ...
  //! primary model update function; the learning param is used to override the
  //! internal learning speed (ignored when <= 0)
  virtual void apply(cv::InputArray image, cv::OutputArray fgmask,
                     double learningRate = 0) = 0;
  virtual void apply(cv::InputArray image,
                     cv::InputArray /*knownForegroundMask*/,
                     cv::OutputArray fgmask, double learningRate = 0) {
  ...
  //! sets the ROI to be used for descriptor extraction (note: this function
  //! will reinit the model and return the usable ROI)
...
protected:
  ...
  //! background model ROI used for LBSP descriptor extraction (specific to the
  //! input image size)
  cv::Mat m_oROI;
  //! input image size
  cv::Size m_oImgSize;
  ...
  //! LBSP relative internal threshold (kept here since we don't keep an LBSP
  //! object)
  const float m_fRelLBSPThreshold;
  //! total number of pixels (depends on the input frame size) & total number of
  //! relevant pixels
  size_t m_nTotPxCount, m_nTotRelevantPxCount;
  //! current frame index, frame count since last model reset & model reset
  //! cooldown counters
  size_t m_nFrameIndex, m_nFramesSinceLastReset, m_nModelResetCooldown;
  //! pre-allocated internal LBSP threshold values LUT for all possible 8-bit
  //! intensities
  size_t m_anLBSPThreshold_8bitLUT[UCHAR_MAX + 1];
  //! internal pixel index LUT for all relevant analysis regions (based on the
  //! provided ROI)
  size_t *m_aPxIdxLUT;
...
private:
  //! copy constructor -- disabled since this class (and its children) use lots
  //! of dynamic structs based on raw pointers
  BackgroundSubtractorLBSP_(const BackgroundSubtractorLBSP_ &);
  //! assignment operator -- disabled since this class (and its children) use
  //! lots of dynamic structs based on raw pointers
